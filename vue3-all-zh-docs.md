...
如果你是有经验的开发者，希望了解如何以最合适的方式在项目中引入 Vue，或者是对上述的这些概念感到好奇，我们在[使用 Vue 的多种方式](undefined)中讨论了有关它们的更多细节。
...
单文件组件是 Vue 的标志性功能。如果你的用例需要进行构建，我们推荐用它来编写 Vue 组件。你可以在后续相关章节里了解更多关于[单文件组件的用法及用途](undefined)。但你暂时只需要知道 Vue 会帮忙处理所有这些构建工具的配置就好。
...
通过组合式 API，我们可以使用导入的 API 函数来描述组件逻辑。在单文件组件中，组合式 API 通常会与 [`<script setup>`](undefined) 搭配使用。这个 `setup` attribute 是一个标识，告诉 Vue 需要在编译时进行一些处理，让我们可以更简洁地使用组合式 API。比如，`<script setup>` 中的导入和顶层变量/函数都能够在模板中直接使用。
...
在[组合式 API FAQ](undefined) 章节中，你可以了解更多关于这两种 API 风格的对比以及组合式 API 所带来的潜在收益。
...
在本节中，我们将介绍如何在本地搭建 Vue [单页应用](undefined)。创建的项目将使用基于 [Vite](https://vitejs.dev) 的构建设置，并允许我们使用 Vue 的[单文件组件](undefined) (SFC)。
...
你现在应该已经运行起来了你的第一个 Vue 项目！请注意，生成的项目中的示例组件使用的是[组合式 API](undefined) 和 `<script setup>`，而非[选项式 API](undefined)。下面是一些补充提示：

- 推荐的 IDE 配置是 [Visual Studio Code](https://code.visualstudio.com/) + [Volar 扩展](https://marketplace.visualstudio.com/items?itemName=Vue.volar)。如果使用其他编辑器，参考 [IDE 支持章节](undefined)。
- 更多工具细节，包括与后端框架的整合，我们会在[工具链指南](undefined)进行讨论。
- 要了解构建工具 Vite 更多背后的细节，请查看 [Vite 文档](https://cn.vitejs.dev)。
- 如果你选择使用 TypeScript，请阅读 [TypeScript 使用指南](undefined)。
...
此命令会在 `./dist` 文件夹中为你的应用创建一个生产环境的构建版本。关于将应用上线生产环境的更多内容，请阅读[生产环境部署指南](undefined)。
...
:::warning 生产环境中的注意事项
到目前为止示例中使用的都是 Vue 的开发构建版本——如果你打算在生产中通过 CDN 使用 Vue，请务必查看[生产环境部署指南](undefined)。
:::
...
组合式 API 的许多示例将使用 `<script setup>` 语法。如果你想在无需构建的情况下使用组合式 API，请参阅 [`setup()` 选项](undefined)。

#### 下一步 {#next-steps}

如果你尚未阅读[简介](undefined)，我们强烈推荐你在移步到后续文档之前返回去阅读一下。

<div class="vt-box-container next-steps">
  <a class="vt-box" href="undefined.html">
    <p class="next-steps-link">继续阅读该指南</p>
    <p class="next-steps-caption">该指南会带你深入了解框架所有方面的细节。</p>
  </a>
  <a class="vt-box" href="/tutorial/">
...
每个 Vue 应用都是通过 [`createApp`](undefined) 函数创建一个新的 **应用实例**：
...
DOM 内模板通常用于[无构建步骤](undefined)的 Vue 应用程序。它们也可以与服务器端框架一起使用，其中根模板可能是由服务器动态生成的。
...
这使得 `TodoDeleteButton` 在应用的任何地方都是可用的。我们会在指南的后续章节中讨论关于组件和其他资源的注册。你也可以在 [API 参考](undefined)中浏览应用实例 API 的完整列表。
...
如果你对虚拟 DOM 的概念比较熟悉，并且偏好直接使用 JavaScript，你也可以结合可选的 JSX 支持[直接手写渲染函数](undefined)而不采用模板。但请注意，这将不会享受到和模板同等级别的编译时优化。
...
双大括号标签会被替换为[相应组件实例中](undefined) `msg` 属性的值。同时每次 `msg` 属性更改时它也会同步更新。

#### 原始 HTML {#raw-html}

双大括号会将数据解释为纯文本，而不是 HTML。若想插入 HTML，你需要使用 [`v-html` 指令](undefined)：
...
双大括号不能在 HTML attributes 中使用。想要响应式地绑定一个 attribute，应该使用 [`v-bind` 指令](undefined)：
...
没有显式包含在列表中的全局对象将不能在模板内表达式中访问，例如用户附加在 `window` 上的属性。然而，你也可以自行在 [`app.config.globalProperties`](undefined) 上显式地添加它们，供所有的 Vue 表达式使用。

#### 指令 Directives {#directives}

指令是带有 `v-` 前缀的特殊 attribute。Vue 提供了许多[内置指令](undefined)，包括上面我们所介绍的 `v-bind` 和 `v-html`。

指令 attribute 的期望值为一个 JavaScript 表达式 (除了少数几个例外，即之后要讨论到的 `v-for`、`v-on` 和 `v-slot`)。一个指令的任务是在其表达式的值变化时响应式地更新 DOM。以 [`v-if`](undefined) 为例：
...
如果你需要传入一个复杂的动态参数，我们推荐使用[计算属性](undefined)替换复杂的表达式，也是 Vue 最基础的概念之一，我们很快就会讲到。
...
之后在讲到 [`v-on`](undefined) 和 [`v-model`](undefined) 的功能时，你将会看到其他修饰符的例子。
...
我们可以使用 [`reactive()`](undefined) 函数创建一个响应式对象或数组：
...
响应式对象其实是 [JavaScript Proxy](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy)，其行为表现与一般对象相似。不同之处在于 Vue 能够跟踪对响应式对象属性的访问与更改操作。如果你对这其中的细节感到好奇，我们在 [深入响应式系统](undefined) 一章中会进行解释，但我们推荐你先读完这里的主要指南。

TypeScript 用户请参阅：[为响应式对象标注类型](undefined) <sup class="vt-badge ts" />
...
若要等待一个状态改变后的 DOM 更新完成，你可以使用 [nextTick()](undefined) 这个全局 API：
...
你也可以直接创建一个[浅层响应式对象](undefined)。它们仅在顶层具有响应性，一般仅在某些特殊场景中需要。
...
`reactive()` 的种种限制归根结底是因为 JavaScript 没有可以作用于所有值类型的 “引用” 机制。为此，Vue 提供了一个 [`ref()`](undefined) 方法来允许我们创建可以使用任何值类型的响应式 **ref**：
...
TypeScript 用户请参阅：[为 ref 标注类型](undefined) <sup class="vt-badge ts" />
...
简言之，`ref()` 让我们能创造一种对任意值的 “引用”，并能够在不丢失响应性的前提下传递这些引用。这个功能很重要，因为它经常用于将逻辑提取到 [组合函数](undefined) 中。
...
只有当嵌套在一个深层响应式对象内时，才会发生 ref 解包。当其作为[浅层响应式对象](undefined)的属性被访问时不会解包。
...
也可参考：[为计算属性标记类型](undefined) <sup class="vt-badge ts" />
...
也可参考：[为计算属性标注类型](undefined) <sup class="vt-badge ts" />
...
计算属性的 getter 应只做计算而没有任何其他的副作用，这一点非常重要，请务必牢记。举例来说，**不要在 getter 中做异步请求或者更改 DOM**！一个计算属性的声明中描述的是如何根据其他值派生一个值。因此 getter 的职责应该仅为计算和返回该值。在之后的指引中我们会讨论如何使用[监听器](undefined)根据其他响应式状态的变更来创建副作用。
...
这也会渲染出相同的结果。我们也可以绑定一个返回对象的[计算属性](undefined)。这是一个常见且很有用的技巧：
...
> 本节假设你已经有 [Vue 组件](undefined)的知识基础。如果没有，你也可以暂时跳过，以后再阅读。
...
你可以在[透传 Attribute](undefined) 一章中了解更多组件的 attribute 继承的细节。
...
::: warning 警告
同时使用 `v-if` 和 `v-for` 是**不推荐的**，因为这样二者的优先级不明显。请查看[风格指南](undefined)获得更多信息。
:::

当 `v-if` 和 `v-for` 同时存在于一个元素上的时候，`v-if` 会首先被执行。请查看[列表渲染指南](undefined)获取更多细节。
...
:::warning 注意
同时使用 `v-if` 和 `v-for` 是**不推荐的**，因为这样二者的优先级不明显。请转阅[风格指南](undefined)查看更多细节。
:::
...
[推荐](undefined)在任何可行的时候为 `v-for` 提供一个 `key` attribute，除非所迭代的 DOM 内容非常简单 (例如：不包含组件或有状态的 DOM 元素)，或者你想有意采用默认行为来提高性能。

`key` 绑定的值期望是一个基础类型的值，例如字符串或 number 类型。不要用对象作为 `v-for` 的 key。关于 `key` attribute 的更多用途细节，请参阅 [`key` API 文档](undefined)。

#### 组件上使用 `v-for` {#v-for-with-a-component}

> 这一小节假设你已了解[组件](undefined)的相关知识，或者你也可以先跳过这里，之后再回来看。
...
你也可以看看[为事件处理器标注类型](undefined)这一章了解更多。<sup class="vt-badge ts" />
...
::: tip 注意
`v-model` 会忽略任何表单元素上初始的 `value`、`checked` 或 `selected` attribute。它将始终将当前绑定的 JavaScript 状态视为数据的正确来源。你应该在 JavaScript 中使用<span class="options-api"> [`data`](undefined) 选项</span><span class="composition-api">[响应式系统的 API](undefined) </span>来声明该初始值。
:::
...
默认情况下，`v-model` 会在每次 `input` 事件后更新数据 ([IME 拼字阶段的状态](undefined)例外)。你可以添加 `lazy` 修饰符来改为在每次 `change` 事件后更新数据：
...
HTML 的内置表单输入类型并不总能满足所有需求。幸运的是，我们可以使用 Vue 构建具有自定义行为的可复用输入组件，并且这些输入组件也支持 `v-model`！要了解更多关于此的内容，请在组件指引中阅读[配合 `v-model` 使用](undefined)。
...
还有其他一些钩子，会在实例生命周期的不同阶段被调用，最常用的是 <span class="composition-api">[`onMounted`](undefined)、[`onUpdated`](undefined) 和 [`onUnmounted`](undefined)。所有生命周期钩子的完整参考及其用法请参考 [API 索引](undefined)。</span><span class="options-api">[`mounted`](undefined)、[`updated`](undefined) 和 [`unmounted`](undefined)。</span>
...
有关所有生命周期钩子及其各自用例的详细信息，请参考<span class="composition-api">[生命周期钩子 API 索引](undefined)</span><span class="options-api">[生命周期钩子 API 索引](undefined)</span>。
...
在选项式 API 中，我们可以使用 [`watch` 选项](undefined)在每次响应式属性发生变化时触发一个函数。
...
在组合式 API 中，我们可以使用 [`watch` 函数](undefined)在每次响应式状态发生变化时触发回调函数：
...
我们可以用 [`watchEffect` 函数](undefined) 来简化上面的代码。`watchEffect()` 允许我们自动跟踪回调的响应式依赖。上面的侦听器可以重写为：
...
你可以参考一下[这个例子](undefined) 的 `watchEffect` 和响应式的数据请求的操作。
...
我们也可以使用组件实例的 [`$watch()` 方法](undefined)来命令式地创建一个侦听器：
...
也可参考：[为模板引用标注类型](undefined) <sup class="vt-badge ts" />
...
TypeScript 用户请参考：[为组件的模板引用标注类型](undefined) <sup class="vt-badge ts" />
...
这和我们嵌套 HTML 元素的方式类似，Vue 实现了自己的组件模型，使我们可以在每个组件内封装自定义内容与逻辑。Vue 同样也能很好地配合原生 Web Component。如果你想知道 Vue 组件与原生 Web Components 之间的关系，可以[阅读此章节](undefined)。

#### 定义一个组件 {#defining-a-component}

当使用构建步骤时，我们一般会将 Vue 组件定义在一个单独的 `.vue` 文件中，这被叫做[单文件组件](undefined) (简称 SFC)：
...
若要将导入的组件暴露给模板，我们需要在 `components` 选项上[注册](undefined)它。这个组件将会以其注册时的名字作为模板中的标签名。
...
当然，你也可以全局地注册一个组件，使得它在当前应用中的任何组件上都可以使用，而不需要额外再导入。关于组件的全局注册和局部注册两种方式的利弊，我们放在了[组件注册](undefined)这一章节中专门讨论。
...
请看 [DOM 模板解析注意事项](undefined)了解更多细节。

#### 传递 props {#passing-props}

如果我们正在构建一个博客，我们可能需要一个表示博客文章的组件。我们希望所有的博客文章分享相同的视觉布局，但有不同的内容。要实现这样的效果自然必须向组件中传递数据，例如每篇文章标题和内容，这就会使用到 props。

Props 是一种特别的 attributes，你可以在组件上声明注册。要传递给博客文章组件一个标题，我们必须在组件的 props 列表上声明它。这里要用到 <span class="options-api">[`props`](undefined) 选项</span><span class="composition-api">[`defineProps`](undefined) 宏</span>：
...
TypeScript 用户请参考：[为组件 props 标注类型](undefined)<sup class="vt-badge ts" />
...
以上就是目前你需要了解的关于 props 的全部了。如果你看完本章节后还想知道更多细节，我们推荐你深入阅读关于 props 的[完整指引](undefined)。
...
子组件可以通过调用内置的 [**`$emit`** 方法](undefined)，通过传入事件名称来抛出一个事件：
...
我们可以通过 <span class="options-api">[`emits`](undefined) 选项</span><span class="composition-api">[`defineEmits`](undefined) 宏</span>来声明需要抛出的事件：
...
这声明了一个组件可能触发的所有事件，还可以对事件的参数进行[验证](undefined)。同时，这还可以让 Vue 避免将它们作为原生事件监听器隐式地应用于子组件的根元素。
...
TypeScript 用户请参考：[为组件 emits 标注类型](undefined)<sup class="vt-badge ts" />
...
以上就是目前你需要了解的关于组件自定义事件的所有知识了。如果你看完本章节后还想知道更多细节，请深入阅读[组件事件](undefined)章节。
...
以上就是目前你需要了解的关于插槽的所有知识了。如果你看完本章节后还想知道更多细节，请深入阅读[组件插槽](undefined)章节。
...
当使用 `<component :is="...">` 来在多个组件间作切换时，被切换掉的组件会被卸载。我们可以通过 [`<KeepAlive>` 组件](undefined)强制被切换掉的组件仍然保持“存活”的状态。
...
自定义的组件 `<blog-post-row>` 将作为无效的内容被忽略，因而在最终呈现的输出中造成错误。我们可以使用特殊的 [`is` attribute](undefined) 作为一种解决方案：
...
> 此章节假设你已经看过了[组件基础](undefined)。若你还不了解组件是什么，请先阅读该章节。
...
在单文件组件和内联字符串模板中，我们都推荐这样做。但是，PascalCase 的标签名在 DOM 模板中是不可用的，详情参见 [DOM 模板解析注意事项](undefined)。
...
一个组件需要显式声明它所接受的 props，这样 Vue 才能知道外部传入的哪些是 props，哪些是透传 attribute (关于透传 attribute，我们会在[专门的章节](undefined)中讨论)。
...
在没有使用 `<script setup>` 的组件中，prop 可以使用 [`props`](undefined) 选项来声明：
...
props 需要使用 [`props`](undefined) 选项来定义：
...
对象形式的 props 声明不仅可以一定程度上作为组件的文档，而且如果其他开发者在使用你的组件时传递了错误的类型，也会在浏览器控制台中抛出警告。我们将在本章节稍后进一步讨论有关 [prop 校验](undefined)的更多细节。
...
更多关于基于类型的声明的细节请参考[组件 props 类型标注](undefined)。<sup class="vt-badge ts" />
...
虽然理论上你也可以在向子组件传递 props 时使用 camelCase 形式 (使用 [DOM 模板](undefined)时例外)，但实际上为了和 HTML attribute 对齐，我们通常会将其写为 kebab-case 形式：

```vue-html
<MyComponent greeting-message="hello" />
```

对于组件名我们推荐使用 [PascalCase](undefined)，因为这提高了模板的可读性，能帮助我们区分 Vue 组件和原生 HTML 元素。然而对于传递 props 来说，使用 camelCase 并没有太多优势，因此我们推荐更贴近 HTML 的书写风格。
...
如果你想要将一个对象的所有属性都当作 props 传入，你可以使用[没有参数的 `v-bind`](undefined)，即只使用 `v-bind` 而非 `:prop-name`。例如，这里有一个 `post` 对象：
...
这种更改的主要缺陷是它允许了子组件以某种不明显的方式影响父组件的状态，可能会使数据流在将来变得更难以理解。在最佳实践中，你应该尽可能避免这样的更改，除非父子组件在设计上本来就需要紧密耦合。在大多数场景下，子组件应该[抛出一个事件](undefined)来通知父组件做出改变。
...
如果使用了[基于类型的 prop 声明](undefined) <sup class="vt-badge ts" />，Vue 会尽最大努力在运行时按照 prop 的类型标注进行编译。举例来说，`defineProps<{ msg: string }>` 会被编译为 `{ msg: { type: String, required: true }}`。
...
像组件与 prop 一样，事件的名字也提供了自动的格式转换。注意这里我们触发了一个以 camelCase 形式命名的事件，但在父组件中可以使用 kebab-case 形式来监听。与 [prop 大小写格式](undefined)一样，在模板中我们也推荐使用 kebab-case 形式来编写监听器。

:::tip
和原生 DOM 事件不一样，组件触发的事件**没有冒泡机制**。你只能监听直接子组件触发的事件。平级组件或是跨越多层嵌套的组件间通信，应使用一个外部的事件总线，或是使用一个[全局状态管理方案](undefined)。
:::
...
组件可以显式地通过 <span class="composition-api">[`defineEmits()`](undefined) 宏</span><span class="options-api">[`emits`](undefined) 选项</span>来声明它要触发的事件：
...
如果你显式地使用了 `setup` 函数而不是 `<script setup>`，则事件需要通过 [`emits`](undefined) 选项来定义，`emit` 函数也被暴露在 `setup()` 的上下文对象上：
...
TypeScript 用户请参考：[如何为组件所抛出事件标注类型](undefined) <sup class="vt-badge ts" />
...
TypeScript 用户请参考：[如何为组件所抛出的事件标注类型](undefined)。<sup class="vt-badge ts" />

</div>

尽管事件声明是可选的，我们还是推荐你完整地声明所有要触发的事件，以此在代码中作为文档记录组件的用法。同时，事件声明能让 Vue 更好地将事件和[透传 attribute](undefined) 作出区分，从而避免一些由第三方代码触发的自定义 DOM 事件所导致的边界情况。
...
在学习输入绑定时，我们知道了 `v-model` 有一些[内置的修饰符](undefined)，例如 `.trim`，`.number` 和 `.lazy`。在某些场景下，你可能想要一个自定义组件的 `v-model` 支持自定义的修饰符。
...
#### Attributes 继承 {undefined}

“透传 attribute”指的是传递给一个组件，却没有被该组件声明为 [props](undefined) 或 [emits](undefined) 的 attribute 或者 `v-on` 事件监听器。最常见的例子就是 `class`、`style` 和 `id`。
...
小提示：[没有参数的 `v-bind`](undefined) 会将一个对象的所有属性都作为 attribute 应用到目标元素上。
...
[动态指令参数](undefined)在 `v-slot` 上也是有效的，即可以定义下面这样的动态插槽名：
...
注意这里的表达式和动态指令参数受相同的[语法限制](undefined)。
...
![scoped slots diagram](undefined)
...
实际上，这已经和作用域插槽的最终代码编译结果、以及手动编写[渲染函数](undefined)时使用作用域插槽的方式非常类似了。
...
虽然这个模式很有趣，但大部分能用无渲染组件实现的功能都可以通过组合式 API 以另一种更高效的方式实现，并且还不会带来额外组件嵌套的开销。之后我们会在[组合式函数](undefined)一章中介绍如何更高效地实现追踪鼠标位置的功能。
...
通常情况下，当我们需要从父组件向子组件传递数据时，会使用 [props](undefined)。想象一下这样的结构：有一些多层级嵌套的组件，形成了一颗巨大的组件树，而某个深层的子组件需要一个较远的祖先组件中的部分数据。在这种情况下，如果仅使用 props 则必须将其沿着组件链逐级传递下去，这会非常麻烦：

![Prop 逐级透传的过程图示](undefined)

<!-- https://www.figma.com/file/yNDTtReM2xVgjcGVRzChss/prop-drilling -->

注意，虽然这里的 `<Footer>` 组件可能根本不关心这些 props，但为了使 `<DeepChild>` 能访问到它们，仍然需要定义并向下传递。如果组件链路非常长，可能会影响到更多这条路上的组件。这一问题被称为“prop 逐级透传”，显然是我们希望尽量避免的情况。

`provide` 和 `inject` 可以帮助我们解决这一问题。 <sup>[[1]](undefined)</sup> 一个父组件相对于其所有的后代组件，会作为**依赖提供者**。任何后代的组件树，无论层级有多深，都可以**注入**由父组件提供给整条链路的依赖。

![Provide/inject 模式](undefined)
...
要为组件后代提供数据，需要使用到 [`provide()`](undefined) 函数：
...
要为组件后代提供数据，需要使用到 [`provide`](undefined) 选项：
...
然而，请注意这**不会**使注入保持响应性。我们会在后续小节中讨论如何[让注入转变为响应式](undefined)。
...
在应用级别提供的数据在该应用内的所有组件中都可以注入。这在你编写[插件](undefined)时会特别有用，因为插件一般都不会使用组件形式来提供值。

#### Inject (注入) {#inject}

<div class="composition-api">

要注入上层组件提供的数据，需使用 [`inject()`](undefined) 函数：
...
要注入上层组件提供的数据，需使用 [`inject`](undefined) 选项来声明：
...
最后，如果你想确保提供的数据不能被注入方的组件更改，你可以使用 [`readonly()`](undefined) 来包装提供的值。
...
为保证注入方和供给方之间的响应性链接，我们需要使用 [computed()](undefined) 函数提供一个计算属性：
...
`computed()` 函数常用于组合式 API 风格的组件中，但它同样还可以用于补充选项式 API 风格的某些用例。你可以通过阅读[响应式系统基础](undefined)和[计算属性](undefined)两个章节了解更多组合式的 API 风格。
...
TypeScript 用户请参考：[为 Provide / Inject 标注类型](undefined) <sup class="vt-badge ts" />
...
在大型项目中，我们可能需要拆分应用为更小的块，并仅在需要时再从服务器加载相关组件。Vue 提供了 [`defineAsyncComponent`](undefined) 方法来实现此功能：
...
与普通组件一样，异步组件可以使用 `app.component()` [全局注册](undefined)：
...
你也可以在[局部注册组件](undefined)时使用 `defineAsyncComponent`：
...
异步组件可以搭配内置的 `<Suspense>` 组件一起使用，若想了解 `<Suspense>` 和异步组件之间交互，请参阅 [`<Suspense>`](undefined) 章节。
...
:::tip
此章节假设你已经对组合式 API 有了基本的了解。如果你只学习过选项式 API，你可以使用左侧边栏上方的切换按钮将 API 风格切换为组合式 API 后，重新阅读[响应性基础](undefined)和[生命周期钩子](undefined)两个章节。
:::
...
如你所见，核心逻辑完全一致，我们做的只是把它移到一个外部函数中去，并返回需要暴露的状态。和在组件中一样，你也可以在组合式函数中使用所有的[组合式 API](undefined)。现在，`useMouse()` 的功能可以在任何组件中轻易复用了。
...
:::tip
每一个调用 `useMouse()` 的组件实例会创建其独有的 `x`、`y` 状态拷贝，因此他们不会互相影响。如果你想要在组件之间共享状态，请阅读[状态管理](undefined)这一章。
:::
...
这个版本的 `useFetch()` 现在同时可以接收静态的 URL 字符串和 URL 字符串的 ref。当通过 [`isRef()`](undefined) 检测到 URL 是一个动态 ref 时，它会使用 [`watchEffect()`](undefined) 启动一个响应式的 effect。该 effect 会立刻执行一次，并在此过程中将 URL 的 ref 作为依赖进行跟踪。当 URL 的 ref 发生改变时，数据就会被重置，并重新请求。
...
尽管其响应性不依赖 ref，组合式函数仍可接收 ref 参数。如果编写的组合式函数会被其他开发者使用，你最好在处理输入参数时兼容 ref 而不只是原始的值。[`unref()`](undefined) 工具函数会对此非常有帮助：
...
- 如果你的应用用到了[服务端渲染](undefined) (SSR)，请确保在组件挂载后才调用的生命周期钩子中执行 DOM 相关的副作用，例如：`onMounted()`。这些钩子仅会在浏览器中被调用，因此可以确保能访问到 DOM。
...
Vue 2 的用户可能会对 [mixins](undefined) 选项比较熟悉。它也让我们能够把组件逻辑提取到可复用的单元里。然而 mixins 有三个主要的短板：
...
在组件插槽一章中，我们讨论过了基于作用域插槽的[无渲染组件](undefined)。我们甚至用它实现了一样的鼠标追踪器示例。
...
如果你有 React 的开发经验，你可能注意到组合式函数和自定义 React hooks 非常相似。组合式 API 的一部分灵感正来自于 React hooks，Vue 的组合式函数也的确在逻辑组合能力上与 React hooks 相近。然而，Vue 的组合式函数是基于 Vue 细粒度的响应性系统，这和 React hooks 的执行模型有本质上的不同。这一话题在[组合式 API 的常见问题](undefined)中有更细致的讨论。

#### 延伸阅读 {#further-reading}

- [深入响应性原理](undefined)：理解 Vue 响应性系统的底层细节。
- [状态管理](undefined)：多个组件间共享状态的管理模式。
- [测试组合式函数](undefined)：组合式函数的单元测试技巧。
...
我们已经介绍了两种在 Vue 中重用代码的方式：[组件](undefined)和[组合式函数](undefined)。组件是主要的构建模块，而组合式函数则侧重于有状态的逻辑。另一方面，自定义指令主要是为了重用涉及普通元素的底层 DOM 访问的逻辑。
...
当在组件上使用自定义指令时，它会始终应用于组件的根节点，和[透传 attributes](undefined) 类似。
...
一个插件可以是一个拥有 `install()` 方法的对象，也可以直接是一个安装函数本身。安装函数会接收到安装它的[应用实例](undefined)和传递给 `app.use()` 的额外选项作为参数：
...
1. 通过 [`app.component()`](undefined) 和 [`app.directive()`](undefined) 注册一到多个全局组件或自定义指令。

2. 通过 [`app.provide()`](undefined) 使一个资源[可被注入](undefined)进整个应用。

3. 向 [`app.config.globalProperties`](undefined) 中添加一些全局实例属性或方法
...
TypeScript 用户请参考：[扩展全局属性](undefined) <sup class="vt-badge ts" />
...
<script setup>
import Basic from 'undefined-demos/Basic.vue'
import SlideFade from 'undefined-demos/SlideFade.vue'
...
- `<TransitionGroup>` 会在一个 `v-for` 列表中的元素或组件被插入，移动，或移除时应用动画。我们将在[下一章节](undefined)中介绍。

除了这两个组件，我们也可以通过其他技术手段来应用动画，比如切换 CSS class 或用状态绑定样式来驱动动画。这些其他的方法会在[动画技巧](undefined)章节中展开。
...
1. Vue 会自动检测目标元素是否应用了 CSS 过渡或动画。如果是，则一些 [CSS 过渡 class](undefined) 会在适当的时机被添加和移除。

2. 如果有作为监听器的 [JavaScript 钩子](undefined)，这些钩子函数会在适当时机被调用。
...
`<Transition>` 也可以作用于[动态组件](undefined)之间的切换：
...
- [过渡模式](undefined)在这里不可用，因为我们不再是在互斥的元素之间进行切换。
...
:::tip
当在 [DOM 模板](undefined)中使用时，组件名需要写为 `<transition-group>`。
:::
...
- [`<TransitionGroup>` API 参考](undefined)


## KeepAlive

<script setup>
import SwitchComponent from 'undefined-demos/SwitchComponent.vue'
</script>
...
在组件基础章节中，我们已经介绍了通过特殊的 `<component>` 元素来实现[动态组件](undefined)的用法：
...
:::tip
在 [DOM 模板](undefined)中使用时，它应该被写为 `<keep-alive>`。
:::
...
它会根据组件的 [`name`](undefined) 选项进行匹配，所以组件如果想要条件性地被 `KeepAlive` 缓存，就必须显式声明一个 `name` 选项。
...
我们可以通过传入 `max` prop 来限制可被缓存的最大组件实例数。`<KeepAlive>` 的行为在指定了 `max` 后类似一个 [LRU 缓存](undefined)>)：如果缓存的实例数量即将超过指定的那个最大数量，则最久没有被访问的缓存实例将被销毁，以便为新的实例腾出空间。
...
一个持续存在的组件可以通过 [`onActivated()`](undefined) 和 [`onDeactivated()`](undefined) 注册相应的两个状态的生命周期钩子：
...
一个持续存在的组件可以通过 [`activated`](undefined) 和 [`deactivated`](undefined) 选项来注册相应的两个状态的生命周期钩子：
...
我们也可以将 `<Teleport>` 和 [`<Transition>`](./transition) 结合使用来创建一个带动画的模态框。你可以看看[这个示例](undefined)。
...
- [`<Teleport>` API 参考](undefined)
- [在 SSR 中处理 Teleports](undefined)
...
`<Suspense>` 组件自身目前还不提供错误处理，不过你可以使用 [`errorCaptured`](undefined) 选项或者 [`onErrorCaptured()`](undefined) 钩子，在使用到 `<Suspense>` 的父组件中捕获和处理异步错误。
...
如你所见，Vue 的单文件组件是网页开发中 HTML、CSS 和 JavaScript 三种语言经典组合的自然延伸。`<template>`、`<script>` 和 `<style>` 三个块在同一个文件中封装、组合了组件的视图、逻辑和样式。完整的语法定义可以查阅 [SFC 语法说明](undefined)。
...
- [让本来就强相关的关注点自然内聚](undefined)
- 预编译模板，避免运行时的编译开销
- [组件作用域的 CSS](undefined)
- [在使用组合式 API 时语法更简单](undefined)
- 通过交叉分析模板和逻辑代码能进行更多编译时优化
- [更好的 IDE 支持](undefined)，提供自动补全和对模板中表达式的类型检查
- 开箱即用的模块热更新 (HMR) 支持
...
在实际项目中，我们一般会使用集成了 SFC 编译器的构建工具，比如 [Vite](https://cn.vitejs.dev/) 或者 [Vue CLI](https://cli.vuejs.org/zh/) (基于 [webpack](https://webpack.js.org/))，Vue 官方也提供了脚手架工具来帮助你尽可能快速地上手开发 SFC。更多细节请查看 [SFC 工具链](undefined)章节。
...
即使你不喜欢单文件组件这样的形式而仍然选择拆分单独的 JavaScript 和 CSS 文件，也没关系，你还是可以通过[资源导入](undefined)功能获得热更新和预编译等功能的支持。
...
具体细节请参考章节：[配合 TypeScript 使用 Vue](undefined)。
...
如果你只需要一个简单的页面路由，而不想为此引入一整个路由库，你可以通过[动态组件](undefined)的方式，监听浏览器 [`hashchange` 事件](https://developer.mozilla.org/en-US/docs/Web/API/Window/hashchange_event)或使用 [History API](https://developer.mozilla.org/en-US/docs/Web/API/History) 来更新当前组件。
...
对于情景 1，一个可行的办法是将共享状态“提升”到共同的祖先组件上去，再通过 props 传递下来。然而在深层次的组件树结构中这么做的话，很快就会使得代码变得繁琐冗长。这会导致另一个问题：[Prop 逐级透传问题](undefined)。
...
在选项式 API 中，响应式数据是用 `data()` 选项声明的。在内部，`data()` 的返回值对象会通过 [`reactive()`](undefined) 这个公开的 API 函数转为响应式。

</div>

如果你有一部分状态需要在多个组件实例间共享，你可以使用 [`reactive()`](undefined) 来创建一个响应式对象，并将它导入到多个组件中：
...
除了我们这里用到的单个响应式对象作为一个 store 之外，你还可以使用其他[响应式 API](undefined) 例如 `ref()` 或是 `computed()`，或是甚至通过一个[组合式函数](undefined)来返回一个全局状态：
...
如果你正在构建一个需要利用[服务端渲染 (SSR)](undefined) 的应用，由于 store 是跨多个请求共享的单例，上述模式可能会导致问题。这在 SSR 指引那一章节会讨论[更多细节](undefined)。
...
还有一个特定用于 Vue 的小节，介绍了组合式函数的测试，详情请参阅[测试组合式函数](undefined)。
...
有一类 Vue 应用中特有的函数被称为 [组合式函数](undefined)，在测试过程中可能需要特殊处理。
你可以跳转到下方查看 [测试组合式函数](undefined) 了解更多细节。
...
2. 黑盒：组件测试
   ...
   黑盒测试不知晓一个组件的实现细节。这些测试尽可能少地模拟，以测试组件在整个系统中的集成情况。它们通常会渲染所有子组件，因而会被认为更像一种“集成测试”。请查看下方的[组件测试建议](undefined)作进一步了解。
...
> 这一小节假设你已经读过了[组合式函数](undefined)这一章。
...
[`renderToString()`](undefined) 接收一个 Vue 应用实例作为参数，返回一个 Promise，当 Promise resolve 时得到应用渲染的 HTML。当然你也可以使用 [Node.js Stream API](https://nodejs.org/api/stream.html) 或者 [Web Streams API](https://developer.mozilla.org/zh-CN/docs/Web/API/Streams_API) 来执行流式渲染。查看 [SSR API 参考](undefined)获取完整的相关细节。
...
为了在激活模式下挂载应用，我们应该使用 [`createSSRApp()`](undefined) 而不是 `createApp()`：
...
该文件及其依赖项在服务器和客户端之间共享——我们称它们为**通用代码**。编写通用代码时有一些注意事项，我们将[在下面讨论](undefined)。
...
在状态管理一章中，我们介绍了一种[使用响应式 API 的简单状态管理模式](undefined)。而在 SSR 环境中，这种模式需要一些额外的调整。
...
推荐的解决方案是在每个请求中为整个应用创建一个全新的实例，包括 router 和全局 store。然后，我们使用[应用层级的 provide 方法](undefined)来提供共享状态，并将其注入到需要它的组件中，而不是直接在组件中将其导入：
...
[应用级错误处理](undefined) 可以用来向追踪服务报告错误：
...
- 查看[使用 Vue 的多种方式](undefined)这一章看看如何用不同的方式围绕 Vue 组织架构。
...
- [Chrome 开发者工具“性能”面板](https://developer.chrome.com/docs/devtools/evaluate-performance/)
  - [`app.config.performance`](undefined) 将会开启 Vue 特有的性能标记，标记在 Chrome 开发者工具的性能时间线上。
- [Vue 开发者扩展](undefined)也提供了性能分析的功能。
...
如果你的用例对页面加载性能很敏感，请避免将其部署为纯客户端的 SPA，而是让服务器直接发送包含用户想要查看的内容的 HTML 代码。纯客户端渲染存在首屏加载缓慢的问题，这可以通过[服务器端渲染 (SSR)](undefined) 或[静态站点生成 (SSG)](undefined) 来缓解。查看 [SSR 指南](undefined)以了解如何使用 Vue 实现 SSR。如果应用对交互性要求不高，你还可以使用传统的后端服务器来渲染 HTML，并在客户端使用 Vue 对其进行增强。
...
懒加载对于页面初次加载时的优化帮助极大，它帮助应用暂时略过了那些不是立即需要的功能。在 Vue 应用中，这可以与 Vue 的[异步组件](undefined)搭配使用，为组件树创建分离的代码块：
...
`v-once` 是一个内置的指令，可以用来渲染依赖运行时数据但无需再更新的内容。它的整个子树都会在未来的更新中被跳过。查看它的 [API 参考手册](undefined)可以了解更多细节。

##### `v-memo` {#v-memo}

`v-memo` 是一个内置指令，可以用来有条件地跳过某些大型子树或者 `v-for` 列表的更新。查看它的 [API 参考手册](undefined)可以了解更多细节。
...
Vue 确实也为此提供了一种解决方案，通过使用 [`shallowRef()`](undefined) 和 [`shallowReactive()`](undefined) 来绕开深度响应。浅层式 API 创建的状态只在其顶层是响应式的，对所有深层的对象不会做任何处理。这使得对深层级属性的访问变得更快，但代价是，我们现在必须将所有深层级对象视为不可变的，并且只能通过替换整个根状态来触发更新：
...
有些时候我们会去创建[无渲染组件](undefined)或高阶组件 (用来渲染具有额外 props 的其他组件) 来实现更好的抽象或代码组织。虽然这并没有什么问题，但请记住，组件实例比普通 DOM 节点要昂贵得多，而且为了逻辑抽象创建太多组件实例将会导致性能损失。
...
![Chrome 开发者工具正在通过标签展示无障碍访问的 input 框的名字](undefined)
...
![Chrome 开发者工具正在通过 aria-label 展示无障碍访问的 input 框名字](undefined)
...
![Chrome 开发者工具通过 aria-labelledby 展示 input 的无障碍访问名称](undefined)
...
![Chrome 开发者工具正在根据 aria-labelledby 和 aria-describedby 展示 input 的无障碍访问名和无障碍访问描述信息](undefined)
...
当一个漏洞被上报时，它会立刻成为我们最关心的问题，会有全职的贡献者暂时搁置其他所有任务来解决这个问题。如需报告漏洞，请发送电子邮件至 [security@vuejs.org](undefined)。
...
为了避免用户的点击被劫持，我们建议仅在沙盒环境的 iframe 中允许用户控制 CSS。或者，当用户控制样式绑定时，我们建议使用其[对象值形式](undefined)并仅允许用户提供能够安全控制的、特定的属性，就像这样：
...
在使用 SSR 时还有一些其他的安全注意事项，因此请确保遵循我们的 [SSR 文档](undefined)给出的最佳实践来避免产生漏洞。
...
- Vue CLI 也提供了对 TypeScript 的支持，但是已经不推荐了。详见[下方的说明](undefined)。
...
为了让 TypeScript 正确地推导出组件选项内的类型，我们需要通过 [`defineComponent()`](undefined) 这个全局 API 来定义组件：
...
- [webpack Treeshaking 的注意事项](undefined)
...
- [`<script setup>` 中针对 TypeScript 的功能](undefined)
...
Vue 也支持使用 JSX / TSX 编写组件。详情请查阅[渲染函数 & JSX](undefined)。

#### 泛型组件 {#generic-components}

泛型组件支持两种使用方式：

- 在单文件组件中：[在 `<script setup>` 上使用 `generic` 属性](undefined)
- 渲染函数 / JSX 组件：[`defineComponent()` 的函数签名](undefined)

#### 特定 API 的使用指南 {#api-specific-recipes}

- [TS 与组合式 API](undefined)
- [TS 与选项式 API](undefined)
...
> 这一章假设你已经阅读了[搭配 TypeScript 使用 Vue](undefined) 的概览。
...
`props` 选项通常用于 Options API，因此你会在[选项式 API 与 TypeScript](undefined) 指南中找到更详细的例子。这些例子中展示的技术也适用于使用 `defineProps()` 的运行时声明。
...
注意，如果你想在 TypeScript 文件而不是在 Vue SFC 中使用这种技巧，需要开启 Volar 的 [Takeover 模式](undefined)。
...
某些插件会通过 [`app.config.globalProperties`](undefined) 为所有组件都安装全局可用的属性。举例来说，我们可能为了请求数据而安装了 `this.$http`，或者为了国际化而安装了 `this.$translate`。为了使 TypeScript 更好地支持这个行为，Vue 暴露了一个被设计为可以通过 [TypeScript 模块扩展](https://www.typescriptlang.org/docs/handbook/declaration-merging.html#module-augmentation)来扩展的 `ComponentCustomProperties` 接口：
...
你可以用 Vue 来[构建标准的 Web Component](undefined)，这些 Web Component 可以嵌入到任何 HTML 页面中，无论它们是如何被渲染的。这个方式让你能够在不需要顾虑最终使用场景的情况下使用 Vue：因为生成的 Web Component 可以嵌入到旧应用、静态 HTML，甚至用其他框架构建的应用中。
...
Vue 提供了核心功能库和[全面的工具链支持](undefined)，为现代 SPA 提供了极佳的开发体验，覆盖以下方面：
...
Vue 提供了一系列 API，支持将一个 Vue 应用在服务端渲染成 HTML 字符串。这能让服务器直接返回渲染好的 HTML，让用户在 JavaScript 下载完毕前就看到页面内容。Vue 之后会在客户端对应用进行“激活 (hydrate)”使其重获可交互性。这被称为[服务端渲染 (SSR)](undefined)，它能够极大地改善应用在 Web 核心指标上的性能表现，如[最大内容绘制 (LCP)](https://web.dev/lcp/)。
...
- 使用 Vue 的[自定义渲染 API](undefined) 来构建不同目标的渲染器，比如 [WebGL](https://troisjs.github.io/) 甚至是[终端命令行](https://github.com/vue-terminal/vue-termui)！
...
- [响应式 API](undefined)：例如 `ref()` 和 `reactive()`，使我们可以直接创建响应式状态、计算属性和侦听器。

- [生命周期钩子](undefined)：例如 `onMounted()` 和 `onUnmounted()`，使我们可以在组件各个生命周期阶段添加逻辑。

- [依赖注入](undefined)：例如 `provide()` 和 `inject()`，使我们可以在使用响应式 API 时，利用 Vue 的依赖注入系统。

组合式 API 是 Vue 3 及 [Vue 2.7](https://blog.vuejs.org/posts/vue-2-7-naruto.html) 的内置功能。对于更老的 Vue 2 版本，可以使用官方维护的插件 [`@vue/composition-api`](https://github.com/vuejs/composition-api)。在 Vue 3 中，组合式 API 基本上都会配合 [`<script setup>`](undefined) 语法在单文件组件中使用。下面是一个使用组合式 API 的组件示例：
...
组合式 API 最基本的优势是它使我们能够通过[组合函数](undefined)来实现更加简洁高效的逻辑复用。在选项式 API 中我们主要的逻辑复用机制是 mixins，而组合式 API 解决了 [mixins 的所有缺陷](undefined)。

组合式 API 提供的逻辑复用能力孵化了一些非常棒的社区项目，比如 [VueUse](https://vueuse.org/)，一个不断成长的工具型组合式函数集合。组合式 API 还为其他第三方状态管理库与 Vue 的响应式系统之间的集成提供了一套简洁清晰的机制，例如[不可变数据](undefined)、[状态机](undefined)与 [RxJS](undefined)。
...
可以。你可以在一个选项式 API 的组件中通过 [`setup()`](undefined) 选项来使用组合式 API。
...
以上代码解释了我们在基础章节部分讨论过的一些 [`reactive()` 的局限性](undefined)：
...
Vue 提供了一个 API 来让你创建响应式副作用 [`watchEffect()`](undefined)。事实上，你会发现它的使用方式和我们上面示例中说的魔法函数 `whenDepsChange()` 非常相似。我们可以用真正的 Vue API 改写上面的例子：
...
实际上，这与 Vue 组件保持状态和 DOM 同步的方式非常接近——每个组件实例创建一个响应式副作用来渲染和更新 DOM。当然，Vue 组件使用了比 `innerHTML` 更高效的方式来更新 DOM。这会在[渲染机制](undefined)一章中详细介绍。
...
Vue 团队确实曾通过一个名为[响应性语法糖](undefined)的实验性功能来探索这个方向，但最后由于[这个原因](https://github.com/vuejs/rfcs/discussions/369#discussioncomment-5059028)，我们认为它不适合这个项目。
...
这两个回调都会作为组件调试的钩子，接受[相同格式](undefined)的调试事件：
...
将 Vue 的响应性系统与外部状态管理方案集成的大致思路是：将外部状态放在一个 [`shallowRef`](undefined) 中。一个浅层的 ref 中只有它的 `.value` 属性本身被访问时才是有响应性的，而不关心它内部的值。当外部状态改变时，替换此 ref 的 `.value` 才会触发更新。
...
虽然这并不是信号的必要特征，但如今这个概念经常与细粒度订阅和更新的渲染模型一起讨论。由于使用了虚拟 DOM，Vue 目前[依靠编译器来实现类似的优化](undefined)。然而，我们也在探索一种新的受 Solid 启发的编译策略 (Vapor Mode)，它不依赖于虚拟 DOM，而是更多地利用 Vue 的内置响应性系统。

##### API 设计权衡 {#api-design-trade-offs}

Preact 和 Qwik 的信号设计与 Vue 的 [shallowRef](undefined) 非常相似：三者都通过 `.value` 属性提供了一个更改接口。我们将重点讨论 Solid 和 Angular 的信号。
...
2. **挂载**：运行时渲染器调用渲染函数，遍历返回的虚拟 DOM 树，并基于它创建实际的 DOM 节点。这一步会作为[响应式副作用](undefined)执行，因此它会追踪其中所用到的所有响应式依赖。

3. **更新**：当一个依赖发生变化后，副作用会重新运行，这时候会创建一个更新后的虚拟 DOM 树。运行时渲染器遍历这棵新树，将它与旧树进行比较，然后将必要的更新应用到真实 DOM 上去。

![render pipeline](undefined)
...
在实践中，模板对大多数的应用场景都是够用且高效的。渲染函数一般只会在需要处理高度动态渲染逻辑的可重用组件中使用。想了解渲染函数的更多使用细节可以去到[渲染函数 & JSX](undefined) 章节继续阅读。
...
更新类型标记和树结构打平都大大提升了 Vue [SSR 激活](undefined)的性能表现：
...
> 如果你还不熟悉虚拟 DOM 和渲染函数的概念的话，请确保先阅读[渲染机制](undefined)章节。
...
没错，这就是一个合法的 Vue 组件！参阅[函数式组件](undefined)来了解更多语法细节。
...
- 传递子元素给组件 (比如 slots) 的[方式不同](undefined)。
...
对于事件和按键修饰符，可以使用 [`withModifiers`](undefined) 函数：
...
如果一个组件是用名字注册的，不能直接导入 (例如，由一个库全局注册)，可以使用 [`resolveComponent()`](undefined) 来解决这个问题。
...
在渲染函数中，可以通过 [this.$slots](undefined) 来访问插槽：
...
诸如 `<KeepAlive>`、`<Transition>`、`<TransitionGroup>`、`<Teleport>` 和 `<Suspense>` 等[内置组件](undefined)在渲染函数中必须导入才能使用：
...
可以使用 [`withDirectives`](undefined) 将自定义指令应用于 vnode：
...
当一个指令是以名称注册并且不能被直接导入时，可以使用 [`resolveDirective`](undefined) 函数来解决这个问题。
...
第二个参数 `context` 包含三个属性：`attrs`、`emit` 和 `slots`。它们分别相当于组件实例的 [`$attrs`](undefined)、[`$emit`](undefined) 和 [`$slots`](undefined) 这几个属性。

</div>

大多数常规组件的配置选项在函数式组件中都不可用，除了 [`props`](undefined) 和 [`emits`](undefined)。我们可以给函数式组件添加对应的属性来声明它们：
...
对于有明确 `props` 的函数式组件，[attribute 透传](undefined)的原理与普通组件基本相同。然而，对于没有明确指定 `props` 的函数式组件，只有 `class`、`style` 和 `onXxx` 事件监听器将默认从 `attrs` 中继承。在这两种情况下，可以将 `inheritAttrs` 设置为 `false` 来禁用属性继承：
...
默认情况下，Vue 会将任何非原生的 HTML 标签优先当作 Vue 组件处理，而将“渲染一个自定义元素”作为后备选项。这会在开发时导致 Vue 抛出一个“解析组件失败”的警告。要让 Vue 知晓特定元素应该被视为自定义元素并跳过组件解析，我们可以指定 [`compilerOptions.isCustomElement` 这个选项](undefined)。
...
Vue 提供了一个和定义一般 Vue 组件几乎完全一致的 [`defineCustomElement`](undefined) 方法来支持创建自定义元素。这个方法接收的参数和 [`defineComponent`](undefined) 完全相同。但它会返回一个继承自 `HTMLElement` 的自定义元素构造器：
...
[Provide / Inject API](undefined) 和[相应的组合式 API](undefined) 在 Vue 定义的自定义元素中都可以正常工作。但是请注意，依赖关系**只在自定义元素之间**起作用。例如一个 Vue 定义的自定义元素就无法注入一个由常规 Vue 组件所提供的属性。
...
- 贪婪 (eager) 的插槽求值会阻碍组件之间的可组合性。Vue 的[作用域插槽](undefined)是一套强大的组件组合机制，而由于原生插槽的贪婪求值性质，自定义元素无法支持这样的设计。贪婪求值的插槽也意味着接收组件时不能控制何时或是否创建插槽内容的节点。

- 在当下要想使用 shadow DOM 书写局部作用域的 CSS，必须将样式嵌入到 JavaScript 中才可以在运行时将其注入到 shadow root 上。这也导致了 SSR 场景下需要渲染大量重复的样式标签。虽然有一些[平台功能](https://github.com/whatwg/html/pull/4898/)在尝试解决这一领域的问题，但是直到现在还没有达到通用支持的状态，而且仍有生产性能 / SSR 方面的问题需要解决。可与此同时，Vue 的 SFC 本身就提供了 [CSS 局域化机制](undefined)，并支持抽取样式到纯 CSS 文件中。
...
Vue 提供了 [`<Transition>`](undefined) 和 [`<TransitionGroup>`](undefined) 组件来处理元素进入、离开和列表顺序变化的过渡效果。但除此之外，还有许多其他制作网页动画的方式在 Vue 应用中也适用。这里我们会探讨一些额外的技巧。
