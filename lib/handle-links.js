...
// async function main(str, confList = [], pkgName) {
async function main(str, paths = [], pkgName) {
  ...
  // let [navConf, sidebarConf] = confList;
  // let paths = [];

  // confList.forEach(conf => {
  //   flatterArr(conf, paths);
  // });
  ...
  let result = list.reduce((arr, path) => {
    ...
    if (pathWithoutRelative.startsWith('#')) {
      arr.push(`${host}${pathWithoutRelative}`);
      return arr;
    }
