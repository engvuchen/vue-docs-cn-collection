...
async function mergeDocs(pkg) {
  ...
  handlePathDiffInOnlineAndFile(paths, pkgName); // 考虑去掉
  ...
  let dataList = await Promise.all(
    ...
    docsConf.map(async pathConf => {
      if (typeof pathConf === 'object') {
        ...
        let resultStr = await handleLinks(str, pathConf, paths, pkgName, filePath);
        return resultStr;
      }
    })
  );
