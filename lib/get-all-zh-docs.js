...
const { getDocsConf, docsConfPathMap, handlePathDiffInOnlineAndFile, flatterArr } = require('./util');
...
async function mergeDocs(pkg) {
  ...
  let docsConf = JSON.parse(JSON.stringify(confList[1]));

  let paths = [];
  confList.forEach(confArr => {
    flatterArr(confArr, paths);
  });
  handlePathDiffInOnlineAndFile(paths, pkgName); // todo 应该放在调用方，现在是每个文章处理一次
  ...
  let dataList = await Promise.all(
    ...
    docsConf.map(async pathConf => {
      if (typeof pathConf === 'object') {
        // 配置是对象
        return childTxtHandler(pathConf, pkgName, paths);
      } else {
        // 配置仅字符串
        // todo
        let filePath = `${accessPathPrefixMap[pkgName]}${pathConf}`;
        let str = readFile(filePath, 'utf-8');
        let resultStr = await handleLinks(str, pathConf, paths, pkgName); // todo
        return resultStr;
      }
    })
  );
...
async function childTxtHandler(pathConf, pkgName, paths = []) {
